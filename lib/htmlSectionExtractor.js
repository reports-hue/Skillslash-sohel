...
// Markdown's spec explicitly allows raw HTML to pass through untouched
// (CommonMark/GFM, and `marked` follows that by design), so a .md import
// can carry a literal <script> or an onerror="..." handler straight into
// contentHtml - which pages/blog/[slug].js renders with
// dangerouslySetInnerHTML, no sanitization anywhere downstream. This is
// the one point every importer's output funnels through on its way into
// that field, so it's sanitized here rather than trusting each importer
// (present or future) to remember to do it themselves. Word/docx content
// doesn't carry this risk by construction (mammoth only ever emits the
// semantic tags it generates, never arbitrary embedded markup), but this
// runs unconditionally for all importers rather than depending on that
// staying true.
const DANGEROUS_TAG_SET = new Set(["script", "style", "iframe", "object", "embed", "link", "meta", "base", "form", "noscript"]);

function sanitizeBodyElements($, elements) {
  // Pass 1: a dangerous tag sitting at the top level (e.g. a bare <script>
  // as its own "paragraph") is dropped from the list entirely, so it's
  // never serialized below - removing it from the live tree alone isn't
  // enough, since $.html(el) serializes the node object directly and
  // doesn't care whether it's still attached to a parent.
  const kept = elements.filter((el) => !DANGEROUS_TAG_SET.has(el.tagName));

  // Pass 2: strip dangerous descendants and attributes from what's left
  // (e.g. an inline <script> inside a <p>, or an onerror="" on an <img>).
  for (const el of kept) {
    const $el = $(el);
    $el.find([...DANGEROUS_TAG_SET].join(", ")).remove();
...
export function extractSections($, children, warnings) {
  ...
  if (sections.body.length) {
    const safeBody = sanitizeBodyElements($, sections.body);
