...
function normLabel(s) {
  // Strips a trailing "(optional - ...)" hint - the sample template puts
  // that guidance in the label itself, never in the value cell (see
  // scripts/generate-docx-template.js), so it has to come off before
  // matching against FIELD_LABEL_MAP.
  return norm(s).toLowerCase().replace(/\s*\([^)]*\)\s*$/, "").replace(/:$/, "");
}
...
export async function parseDocxToPost(buffer) {
  ...
  for (const el of children) {
    ...
    if (/^h[1-6]$/.test(el.tagName)) {
      const target = SECTION_HEADINGS[normLabel($(el).text())];
      if (target) {
        current = target;
        continue;
      }
